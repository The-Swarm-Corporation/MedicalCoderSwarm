...
class DatabaseOps:
    ...
    async def store_message(
        ...
        stmt = select(Conversation).where(Conversation.user_id == str(user_id))
...
class ContextAwareMedicalSwarm:
    ...
    async def process_with_context(
        ...
        try:
            history = await DatabaseOps.get_conversation_history(db, self.user_id)
            ...
            logger.error(f"Swarm processing error for user {self.user_id}: {str(e)}")
...
class MedicalBot(commands.Bot):
    ...
    def check_rate_limit(self, user_id: str) -> bool:
        ...
        return len(self.rate_limits[user_id]) < config.MAX_REQUESTS_PER_MINUTE
...
@bot.tree.command(name="analyze", description="Analyze medical text for coding")
async def analyze_command(interaction: discord.Interaction, text: str):
    ...
    async with bot.db_session() as db:
        try:
            ...
            # Store bot response
            await DatabaseOps.store_message(db, user_id, response, "assistant")
...
async def clear_command(interaction: discord.Interaction):
    ...
    async with bot.db_session() as db:
        try:
            await DatabaseOps.clear_history(db, str(interaction.user.id))
            await interaction.response.send_message(
...
async def get_conversation(
    ...
    """Get conversation history for a user."""
    try:
        history = await DatabaseOps.get_conversation_history(db, user_id)
