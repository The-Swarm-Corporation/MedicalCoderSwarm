...
def fetch_patient_data(patient_id: str) -> Optional[str]:
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    cursor.execute("SELECT patient_data FROM patients WHERE patient_id = ?", (patient_id,))
    row = cursor.fetchone()
    connection.close()
...
def run_medical_coder(patient_case: PatientCase):
    ...
    """
    swarm = MedicalCoderSwarm(patient_id=patient_case.patient_id, max_loops=1, patient_documentation="")
    swarm.run(task=patient_case.case_description)

    swarm_output = swarm.to_dict()
    save_patient_data(patient_case.patient_id, json.dumps(swarm_output))

    return QueryResponse(patient_id=patient_case.patient_id, case_data=json.dumps(swarm_output))

@app.get("/v1/medical-coder/patient/{patient_id}", response_model=QueryResponse)
def get_patient_data(patient_id: str):
    ...
    return QueryResponse(patient_id=patient_id, case_data=patient_data)

@app.get("/v1/medical-coder/patients", response_model=QueryAllResponse)
def get_all_patients():
    ...
    patients = [QueryResponse(patient_id=row[0], case_data=row[1]) for row in rows]
...
def delete_patient_data(patient_id: str):
    ...
    return {"message": "Patient data deleted successfully", "patient_id": patient_id}
