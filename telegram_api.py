...
app = FastAPI(title="Telegram Medical Bot API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class BotConfig:
    def __init__(self):
        self.TELEGRAM_TOKEN = "7647014697:AAGoYyaa0deQSy0mXHZgs78OB9XPMoBEQbg"
...
class ContextAwareMedicalSwarm:
    ...
    async def process_with_context(
        ...
        try:
            ...
            history = await DatabaseOps.get_conversation_history(db, self.chat_id)
            ...
            logger.error(f"Swarm processing error for chat {self.chat_id}: {str(e)}")
            return "I apologize, but I couldn't process your request at this time."

# Telegram bot handler class
class TelegramBotHandler:
    def __init__(self):
        self.bot = Application.builder().token(config.TELEGRAM_TOKEN).build()
        self.setup_handlers()
    ...
    def setup_handlers(self):
        ...
        self.bot.add_handler(CommandHandler("clear", self.clear_command))
    ...
    def check_rate_limit(self, chat_id: int) -> bool:
        ...
        return len(self.rate_limits[chat_id]) < config.MAX_REQUESTS_PER_MINUTE
    ...
    async def handle_message(
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                response = await swarm.process_with_context(user_message, db)
...
async def start_bot(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Start bot
        background_tasks.add_task(bot_handler.bot.run_polling)
...
async def get_conversation(
    ...
    try:
        history = await DatabaseOps.get_conversation_history(db, chat_id)
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        port=8000,
        # reload=True,
        workers=10  # Use 1 worker for Telegram bot
    )