...
class TwitterConfig:
    def __init__(self):
        ...
        self.MAX_REQUESTS_PER_WINDOW = 180  # Twitter's rate limit for most endpoints
...
class Storage:
    ...
    def check_rate_limit(self, endpoint: str) -> bool:
        ...
        return len(self.rate_limits[endpoint]) < config.MAX_REQUESTS_PER_WINDOW
...
class TwitterClient:
    ...
    async def reply_to_tweet(self, tweet_id: str, user: str, message: str) -> bool:
...
async def process_medical_coding(tweet_id: str, content: str) -> str:
    ...
    try:
        ...
        logger.error(f"Medical coding failed for tweet {tweet_id}: {str(e)}")
...
async def process_mention(
    ...
    if keywords and not any(keyword.lower() in content.lower() for keyword in keywords):
        logger.info(f"Tweet {tweet_id} doesn't contain any keywords, skipping")
        return
        
    try:
        # Process medical coding
        response_data = await process_medical_coding(tweet_id, content)
        ...
        logger.error(f"Failed to process mention {tweet_id}: {str(e)}")
        # Store failed task
        task = Task(
            tweet_id=str(tweet_id),
            user=user,
    ...
    while True:
        try:
            ...
            for mention in mentions:
                await process_mention(mention, response_template, keywords)
...
async def start_polling(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to start polling"
        )
